	...
	<?php endif; ?>
		...
		<?php // Loop which loads the slideshow
			...
			if ( $meteor_count == 1 ) {
				...
				$meteor_shim = wp_get_attachment_image_src( get_post_thumbnail_id($slide->ID), 'featured-slide');
			...
			<div class="mslide mslide-<?php echo $meteor_count; ?>">
				...
				if ( get_post_meta( $slide->ID, "slide_url_value", $single = true ) != "" ): ?>
						
					<a href="<?php echo get_post_meta( $slide->ID, "slide_url_value", $single = true ); ?>" title="<?php $slide->post_title ?>"><?php get_the_post_thumbnail( $slide->ID,'featured-slide', array( 'title' => $slide->post_title ) ); ?></a>
			
				<?php // Adds slide image without Slide URL link
					
				else: ?>
					
					<?php echo get_the_post_thumbnail( $slide->ID, 'featured-slide', array( 'title' => $slide->post_title ) ); ?>
