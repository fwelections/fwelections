...
class WikiWidget extends WP_Widget {
	...
	$widget_ops = array( 'description' => __('Display Wiki Pages', $this->translation_domain) );
        ...
        add_shortcode('wikiindex', array($this, 'WikiIndex_Shortcode'));
	parent::WP_Widget( 'incsub_wiki', __('Wiki', $this->translation_domain), $widget_ops, $control_ops );
    }
    
    
    public function WikiIndex_Shortcode($atts) {
		...
		?>
                <li><a href="<?php print get_permalink($wiki->ID); ?>" class="<?php print ($wiki->ID == $post->ID)?'current':''; ?>" ><?php print translate($wiki->post_title); ?></a>
...
//        var_dump($instance);
	...
	<ul>
	    ...
	    ?>
            <li><a href="<?php print get_permalink($sub_wiki->ID); ?>" class="<?php print ($sub_wiki->ID == $post->ID)?'current':''; ?>" ><?php print translate($sub_wiki->post_title); ?></a>
