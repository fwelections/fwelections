...
class modal_dialog_plugin {
	...
	function modal_dialog_header($manualdisplay = false) {
		...
		$thePostID = $post->ID;
                ...
                if (!$this->must_show_lightbox()) return false;
	...
	function modal_dialog_footer( $manualdisplay = false ) {
		...
		if ($display == true && $dialogid != 0)
		{
			...
			if ($options['contentlocation'] == 'Inline')
			{
				...
				$output .= '<div align="center"><a href="'.admin_url( 'profile.php' ).'" ><h1>'.  dg_get_registration_reminder_text().'</h1></a></div>';
				$output .= "</div></div>\n";
			}
	...
	public function admin_enqueue_scripts()
	{
        ...
        public function must_show_lightbox() {
            if (!is_user_logged_in()) return false;
                
            $author_organization = get_user_meta(get_current_user_id(), 'author_organization');
            $author_country = get_user_meta(get_current_user_id(), 'author_country');
            $author_city = get_user_meta(get_current_user_id(), 'author_city');

            if (  (strlen($author_organization[0]) != 0) && (strlen($author_country[0]) != 0) && (strlen($author_city[0]) != 0) ) return false;
            else return true;
